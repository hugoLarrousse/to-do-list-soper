...
type ProTabNavigationProp = CompositeNavigationProp<
  MaterialTopTabNavigationProp<ListsTabsParamList, 'Pro'>,
...
export default function ProTab() {
  const navigation = useNavigation<ProTabNavigationProp>();
  const [actions, setActions] = useState<Action[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  const loadActions = useCallback(async () => {
    const data = await actionRepo.getActiveByList(ActionList.Pro);
    setActions(data);
  }, []);
  ...
  const handlePressAdd = useCallback(() => {
    navigation.navigate('AddAction', { list: ActionList.Pro });
  }, [navigation]);
